...
# Class based view for add product
...
# Class based view for update product
...
# Class based view for delete product
...
def create_checkout_session(request, id):
    ...
    print(f"Saving OrderDetail with payment intent ID: {order.stripe_payment_intent}")  # Add this line
    order.amount = int(product.price*100)
    order.save()
    print(f"Order saved: {order.pk}")
...
class PaymentSuccessView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        print(
            f"Payment intent ID: {session.payment_intent}"
        )  # Add this line to print the payment intent ID
        try:
            ...
            )  # Add this line to print when the OrderDetail is not found
            return HttpResponseNotFound()
        order.has_paid = True
        order.save()
